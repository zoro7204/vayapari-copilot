...
import { getExpensesData, createNewExpense, deleteExpense, updateExpense, getExpensesCsv } from '../services/api';
...
const ExpenseManagement: React.FC = () => {
  ...
  // --- NEW CODE: State for date filtering ---
  ...
  // ... (all handler functions for modals and sorting remain the same) ...
  ...
  // --- MODIFIED CODE: Main logic now includes date filtering ---
  ...
  // ... (summary stat calculations remain the same) ...
    ...
    <div className="p-6 bg-gray-50 min-h-full">
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
            <button className="bg-gray-200 text-gray-800 px-4 py-3 rounded-lg font-medium hover:bg-gray-300 transition-colors flex items-center space-x-2">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-6 flex justify-between items-center">
        {/* --- MODIFIED CODE: Search bar is now smaller --- */}
        ...
        {/* --- NEW CODE: Date Filter Buttons --- */}
        <div className="flex items-center space-x-2">
            <button onClick={() => setDateFilter('today')} className={dateFilter === 'today' ? 'bg-indigo-600 text-white px-4 py-2 rounded-lg' : 'bg-gray-200 text-gray-700 px-4 py-2 rounded-lg'}>Today</button>
            <button onClick={() => setDateFilter('week')} className={dateFilter === 'week' ? 'bg-indigo-600 text-white px-4 py-2 rounded-lg' : 'bg-gray-200 text-gray-700 px-4 py-2 rounded-lg'}>This Week</button>
            <button onClick={() => setDateFilter('month')} className={dateFilter === 'month' ? 'bg-indigo-600 text-white px-4 py-2 rounded-lg' : 'bg-gray-200 text-gray-700 px-4 py-2 rounded-lg'}>This Month</button>
            <button onClick={() => setDateFilter('all')} className={dateFilter === 'all' ? 'bg-indigo-600 text-white px-4 py-2 rounded-lg' : 'bg-gray-200 text-gray-700 px-4 py-2 rounded-lg'}>All Time</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="overflow-x-auto">
            ...
            {/* ... (Table header remains the same with sorting buttons) ... */}
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left py-4 px-6 font-semibold text-gray-800">
                  <button onClick={() => requestSort('expenseId')} className="flex items-center space-x-1">
                    <span>Expense ID</span>
                    <ArrowUpDown className="h-4 w-4 text-gray-400" />
                  </button>
                </th>
                <th className="text-left py-4 px-6 font-semibold text-gray-800">
                  <button onClick={() => requestSort('item')} className="flex items-center space-x-1">
                    <span>Item/Reason</span>
                     <ArrowUpDown className="h-4 w-4 text-gray-400" />
                  </button>
                </th>
                <th className="text-left py-4 px-6 font-semibold text-gray-800">
                   <button onClick={() => requestSort('category')} className="flex items-center space-x-1">
                    <span>Category</span>
                     <ArrowUpDown className="h-4 w-4 text-gray-400" />
                  </button>
                </th>
                ...
                <th className="text-right py-4 px-6 font-semibold text-gray-800">
                   <button onClick={() => requestSort('amount')} className="flex items-center space-x-1 ml-auto">
                    <span>Amount</span>
                     <ArrowUpDown className="h-4 w-4 text-gray-400" />
                  </button>
                </th>
            ...
            <tbody className="divide-y divide-gray-200">
              {/* --- MODIFIED CODE: Use the new processedExpenses variable --- */}
                ...
                <tr key={expense.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-4 px-6 text-center">
                    <div className="flex items-center justify-center space-x-2">
                      <button onClick={() => handleOpenEditExpenseModal(expense)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit className="h-4 w-4" /></button>
                      <button onClick={() => handleDeleteExpense(expense.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {processedExpenses.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400 mt-1">Try creating a new expense or adjusting your search.</p>
        </div>
      )}
